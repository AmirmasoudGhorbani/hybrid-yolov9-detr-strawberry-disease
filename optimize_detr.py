"""
optimize_detr.py — Stage 4 of the pipeline.

Fine-tunes `detr_strawberry_model_2` (from train_detr.py) with lower learning rates and a
StepLR scheduler, saving `detr_strawberry_model_finetuned`. Also reports final losses and
plots a confusion matrix for the fine-tuned model.
...
class Detr(pl.LightningModule):
    def __init__(self, lr, lr_backbone, weight_decay):
        super().__init__()
        # Load the DETR trained in stage 3 (train_detr.py)
        self.model = DetrForObjectDetection.from_pretrained(
            "/content/drive/MyDrive/detr_strawberry_model_2",
...
if __name__ == "__main__":
    ...
    model.model.save_pretrained("/content/drive/MyDrive/detr_strawberry_model_finetuned")
    processor.save_pretrained("/content/drive/MyDrive/detr_strawberry_processor_finetuned")

    analyze_model_performance(trainer)
    plot_confusion_matrix("/content/drive/MyDrive/detr_strawberry_model_finetuned", val_dataloader)

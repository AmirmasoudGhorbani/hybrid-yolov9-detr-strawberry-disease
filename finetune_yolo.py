"""
finetune_yolo.py — Stage 2 of the pipeline.

Resumes from the YOLOv9 `best.pt` produced by `train_yolo.py` and fine-tunes it with a
lower learning rate and early stopping disabled, producing the final YOLO weights used by
`hybrid_inference.py` (stage 7).
...
if __name__ == "__main__":
    ...
    # Load the YOLOv9s model trained in stage 1 (train_yolo.py)
    model = YOLO('/content/drive/MyDrive/yolov9_lastfinetune_results/A100_experiment/weights/best.pt')
    # Fine-tune with a lower learning rate and early stopping disabled
    model.train(

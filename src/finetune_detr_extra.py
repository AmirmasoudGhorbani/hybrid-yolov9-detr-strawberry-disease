"""
Stage 5 — Extra fine-tuning pass on detr_strawberry_model_finetuned (from stage 4),
producing detr_strawberry_model_extrafinetuned — the final DETR weights used for
evaluation (stage 6) and in the hybrid model (stage 7).
"""
...
from src.config import (
    ...
    DETR_MODEL_FINETUNED, DETR_MODEL_EXTRAFINETUNED, DETR_PROCESSOR_EXTRAFINETUNED,
)
...
if __name__ == "__main__":
    ...
    model = Detr(
        model_name_or_path=DETR_MODEL_FINETUNED,
    ...
    model.model.save_pretrained(DETR_MODEL_EXTRAFINETUNED)
    processor.save_pretrained(DETR_PROCESSOR_EXTRAFINETUNED)

    print_final_metrics(trainer)
    plot_confusion_matrix(DETR_MODEL_EXTRAFINETUNED, val_dataloader)
